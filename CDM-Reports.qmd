...
```{r}
#| label: Directory
edc_path <- dir_ls(gsub("\\\\", "/",path), recurse = T, regexp = ".sas7bdat$") |> 
...
```{r}
#| label: Create Dir on M Drive
# if (!dir_exists(str_c(str_replace(edc_path[1],"OpenClinica.+","CDM_Reports"),"/",format(Sys.Date(),"%Y%m%d")))){
#     dir_create(str_c(str_replace(edc_path[1],"OpenClinica.+","CDM_Reports"),"/",format(Sys.Date(),"%Y%m%d")))
# }
```
